-- Restaurant / Founding: 500 SMS/month included (was 800).
-- Sync venues, current-period sms_usage rows, and increment_sms_usage fallbacks.

UPDATE venues
SET sms_monthly_allowance = 500
WHERE LOWER(TRIM(COALESCE(pricing_tier, ''))) IN ('restaurant', 'founding');

UPDATE sms_usage su
SET
  messages_included = 500,
  overage_count = GREATEST(0, su.messages_sent - 500),
  overage_amount_pence = GREATEST(0, su.messages_sent - 500) * COALESCE(su.overage_rate_pence, 6),
  updated_at = now()
FROM venues v
WHERE su.venue_id = v.id
  AND LOWER(TRIM(COALESCE(v.pricing_tier, ''))) IN ('restaurant', 'founding');
...
BEGIN
  ...
  SELECT
    ...
    COALESCE(sms_monthly_allowance,
      CASE
        WHEN LOWER(TRIM(COALESCE(pricing_tier, ''))) IN ('restaurant', 'founding') THEN 500
        WHEN LOWER(TRIM(COALESCE(pricing_tier, ''))) = 'appointments' THEN 500
        WHEN LOWER(TRIM(COALESCE(pricing_tier, ''))) = 'plus' THEN 250
        WHEN LOWER(TRIM(COALESCE(pricing_tier, ''))) = 'light' THEN 100
        ELSE 500
      END
    )
  INTO v_tier, v_allow
