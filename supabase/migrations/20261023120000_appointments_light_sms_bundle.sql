-- Appointments Light: 100 SMS/month included, 6p overage (same as other appointment tiers).
-- Sync increment_sms_usage fallbacks with application constants.

UPDATE venues
SET sms_monthly_allowance = 100
WHERE LOWER(TRIM(COALESCE(pricing_tier, ''))) = 'light';

UPDATE venues
SET sms_monthly_allowance = 250
WHERE LOWER(TRIM(COALESCE(pricing_tier, ''))) = 'plus';
