...
function parseShortConfirmCode(code: string): string | null {
  ...
  const expectedFull = createHmac('sha256', secret).update(`confirm:${payload}`).digest('base64url');
...
export async function GET(
  ...
  const bookingId = parseShortConfirmCode(code);
  if (!bookingId) {
  ...
  return NextResponse.redirect(
    new URL(`/confirm/${booking.id}?hmac=${encodeURIComponent(hmac)}`, baseUrl),
  );
}
