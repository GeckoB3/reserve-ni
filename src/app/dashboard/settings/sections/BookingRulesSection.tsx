...
export function BookingRulesSection({
  ...
  const onAppointmentSubmit = useCallback(
    async (data: AppointmentForm) => {
      const merged: Record<string, unknown> = { ...rules, ...data };
      delete merged.cancellation_notice_hours_by_model;
      delete merged.reminder_hours_before_by_model;
      ...
      <section className="rounded-lg border border-neutral-200 bg-white p-6 shadow-sm">
        ...
        <form onSubmit={handleSubmit(onAppointmentSubmit)} className="space-y-4 max-w-2xl">
          ...
          <div>
            ...
            <p className="mt-1 text-xs text-neutral-500">How many hours before the start time a client must cancel to receive a deposit refund (applies to all booking types)</p>
