...
const PlanSchema = z.enum(['appointments', 'plus', 'light', 'restaurant', 'founding']);
...
const PostBodySchema = z
  ...
  .refine(
    (data) => {
      if (data.plan === 'appointments' || data.plan === 'plus' || data.plan === 'light') return true;
...
export async function POST(request: Request) {
  try {
    ...
    if (plan === 'appointments' || plan === 'plus' || plan === 'light') {
