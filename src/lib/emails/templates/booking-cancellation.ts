...
export function renderBookingCancellation(
  ...
  const mainContent = appt
    ? '<p style="margin:0 0 12px 0">Your appointment has been cancelled.</p>'
    : '<p style="margin:0 0 12px 0">Your reservation has been cancelled.</p>';
...
export function renderBookingCancellationSms(
  ...
  if (appt) {
    return {
      body: `${venue.name}: Your appointment on ${date} at ${time} has been cancelled.${refundPart} We hope to see you another time.`,
    };
  }
  return {
