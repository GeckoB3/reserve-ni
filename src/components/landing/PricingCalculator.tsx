...
type PricingCalculatorProps = {
  /** Controlled: calendar / team member count (e.g. signup plan step). */
  count?: number;
  onCountChange?: (n: number) => void;
};

export function PricingCalculator(props: PricingCalculatorProps = {}) {
  const { count: controlledCount, onCountChange } = props;
  const [internalCount, setInternalCount] = useState(1);
  const isControlled = controlledCount !== undefined && onCountChange !== undefined;
